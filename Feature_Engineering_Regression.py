...
for brand in all_categories:
    ...
    def add_features(df, discount_index):
    ...
    #    means_1year = window_1year.mean()
        ...
        dataframe_final = dataframe.ix[discount_index]
    ...
    discount_index = df[(df['Discount']!= str(0.0))].index.tolist() 
    X = add_features(df, discount_index)
    
    X = X.replace(np.nan, 0)
    Y = df['Discount_off'].ix[discount_index]
    ...
    PIK = "Data/" + brand + "_Regression.dat"
